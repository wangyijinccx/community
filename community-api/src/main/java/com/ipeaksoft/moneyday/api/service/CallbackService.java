...
public class CallbackService extends BaseService {
	...
	private static final String CALLBACL_URL = "http://ads.i43.com/api/active?taskSource=%s&idfa=%s&appID=%s";
	...
	@Async
	public void activeAsync(TaskFast task, FastActive fastActive){
		active(task,fastActive);
	}
	 ...
	 */
	@Async
	public void toChannel(TaskFast fast,FastClick click){
		Date now = new Date();
    ...
    public boolean query(TaskFast task, String idfa, FastClick fastClick){
    	...
    	case "DIANLE":
    		...
    		break;
    	case "KAIWAN":
    		url = String.format(duplicate_url,  idfa, task.getAppid(), task.getAdId(), fastClick.getClientIP());
    		break;
    	case "KUAIYOU":
    		url = duplicate_url;
    		break;
    	...
    	case "JUDIAN":
			...
			break;
    	case "TONGBAN":
			url = String.format(duplicate_url, idfa,fastClick.getClientIP());
			break;
    	case "HAOXQ":
			url = String.format(duplicate_url, idfa);
			break;
    	case "PP":
			...
			break;
    	case "CHUJIN":
			url = String.format(duplicate_url,task.getAdId(), idfa,fastClick.getOSVersion(),fastClick.getClientIP());
			break;
    	case "BYB":
			url = String.format(duplicate_url,task.getAppid(), idfa);
			break;
    	case "YITUILZ":
    		url = String.format(duplicate_url, idfa, "7032",callbackUrl);
    		break;
    	case "WEIWEI":
    		url = String.format(duplicate_url, idfa);
    		break;
    	...
    	if (StringUtils.isNotBlank(url)){
        	if ("YOUMI".equals(task.getTaskSource())){
        		...
        		@SuppressWarnings("unchecked")
    			Map<String, Object> res = JSONObject.parseObject(result, Map.class);
    			if (res != null && !"0".equals((String) res.get("msg"))) {
    				return true;
    			}
        	}	
        	...
        	else if ("HAOXQ".equals(task.getTaskSource())){
        		String result = httpService.get(url);
        		@SuppressWarnings("unchecked")
    			Map<String, Object> res = JSONObject.parseObject(result, Map.class);
        	...
        	else if ("BYB".equals(task.getTaskSource())){
        		String result = httpService.get(url);
    			//Map<String, Object> res = JSONObject.parseObject(result, Map.class);
        		JSONObject json = JSONObject.parseObject(result);
        		JSONArray tables = json.getJSONArray("Content");
        		JSONObject val = (JSONObject) tables.get(0);
    			if (val != null && 1 == (Integer) val.get("Value")) {
    				return true;
    			}
        	}
        	else if ("YITUILZ".equals(task.getTaskSource())){
        		String result = httpService.get(url);
            	JSONObject json = JSONObject.parseObject(result);
            	return !"200".equals(json.getString("code"));
        	}
        	else if ("WEIWEI".equals(task.getTaskSource())){
        		String result = httpService.get(url);
            	JSONObject json = JSONObject.parseObject(result);
            	return !"1".equals(json.getString("success"));
        	}
        	else {
            	String result = httpService.get(url);
            	JSONObject json = JSONObject.parseObject(result);
     ...
     */
    @Async
    public void clickAsync(TaskFast task, String idfa, FastClick fastClick,String signForShanDai){
    	clickSync(task, idfa, fastClick,signForShanDai);
    }
     ...
     * @param fastClick
     */
    public boolean clickSync(TaskFast task, String idfa, FastClick fastClick,String signForShanDai){
        if (!map.containsKey(task.getTaskSource())){
    		return false;
    	}
		...
		switch (task.getTaskSource()){
    	case "LANMAOG":
    		url = String.format(click_url, adid, idfa,ip,fastClick.getOSVersion(),callbackUrl);
    		break;
    	case "KEJINXINXI":
    	...
    	case "CHANDASHI":
			...
			break;
    	case "MOPAN":
			url = String.format(click_url, "i43", adid, mac, idfa, callbackUrl, ip);
			break;
    	case "HUDONG":
    	...
    	case "YITUI":
    		...
    		break;
    	case "JUDIAN":
    		String appKey = "5FB85129CD144580AB17BC53361E";
    		String sign =appKey+"adid"+task.getAdId()+"callback"+callbackUrl+"cuid101idfa"+idfa+"ip"+fastClick.getClientIP()+"mac02:00:00:00:00:00udid";
    		sign = MD5Util.md5(sign);
    		url = String.format(click_url, task.getAdId(), idfa,fastClick.getClientIP(),callbackUrl,sign);
    		break;
    	case "AMIBAK":
    		String[] osVersion={"9.0.2","10.1","9.3.1","10.0"};
    		int i = (int) Math.floor(Math.random()*4);
    		url = String.format(click_url,task.getAdId(), idfa, fastClick.getClientIP(), osVersion[i],callbackUrl);
    		break;
    	case "QINGMI":
    		url = String.format(click_url, task.getAdId(), idfa, ip, callbackUrl);
    		break;
    	case "HAOXQ":
			url = String.format(click_url, idfa);
			break;
    	case "QUANMIN":
			url = String.format(click_url, idfa);
			break;
    	case "SHANDAI":
			url = String.format(click_url, task.getAppid(), idfa,
					signForShanDai, ip);
			break;
    	case "PP":
			url = String.format(click_url,task.getAppid(), idfa,fastClick.getClientIP(),callbackUrl);
			break;
    	case "CHUJIN":
			url = String.format(click_url,task.getAdId(), idfa,fastClick.getOSVersion(),fastClick.getClientIP(),callbackUrl);
			break;
    	}
    	if (StringUtils.isNotBlank(url)){
    		boolean ret = false;
     ...
     * @param idfa
     */
    public boolean active(TaskFast task, FastActive fastActive){
    	if (!map.containsKey(task.getTaskSource())){
    		return false;
    	}
	    ...
	    case "SHANGBANG2":
			...
			break;
	    case "ZHONGYI":
    		url = String.format(active_url, task.getAdId(),fastActive.getClientIP(),idfa);
    		break;
	    case "JUDIAN":
    		String appKey = "5FB85129CD144580AB17BC53361E";
    		String sign =appKey+"adid"+task.getAdId()+"cuid101idfa"+idfa+"ip"+fastActive.getClientIP()+"mac02:00:00:00:00:00";
    		sign = MD5Util.md5(sign);
    		url = String.format(active_url, task.getAdId(), idfa,fastActive.getClientIP(),sign);
    		break;
	    case "BAIHE":
    		url = String.format(active_url, task.getAdId(),fastActive.getClientIP(),idfa);
    		break;
    	...
    	if (StringUtils.isNotBlank(url)){
    		if("CHANDASHI".equals(task.getTaskSource())){
    			String result = httpService.get(url);
            	JSONObject json = JSONObject.parseObject(result);
            	return "success".equals(json.getString("message"));
    		}
    		httpService.get(url);
    	}
    	return true;
    ...
    @Async
    public void sendXYAd(TaskFast task, String type,FastClick fastClick){
		String url = null;
		String xyId = task.getXyId();
		String idfa = fastClick.getIdfa();
		String ip = fastClick.getClientIP();
		if(StringUtils.isBlank(xyId)){
			return;
		}
		AdsSource ad = map.get("XINGYUN");
		switch (type) {
		case "click":
			String click_url = ad.getClickUrl();
			url = String.format(click_url, idfa, xyId,ip);
			break;
		case "callback":
			//类似激活上报了
			String active_url =ad.getActiveUrl();
			url = String.format(active_url, idfa, xyId);
			break;
		}
		httpService.get(url);
	}
     ...
     * @return
     */
    public String queryForShanDai(TaskFast task, String idfa, FastClick fastClick){
    	if (!task.isDuplicate()){
    		return "NO";
    	}
    	if (!map.containsKey(task.getTaskSource())){
    	...
    	String url = String.format(duplicate_url,task.getAppid(), idfa);
    	String result = httpService.get(url);
    	JSONObject json = JSONObject.parseObject(result);
    	//如果服务器异常 比如超时，就报idfa重复吧
    	if("0".equals(json.getString(idfa))){
